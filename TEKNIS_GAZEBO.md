...
ke cart sebagai dudukan bearing, bukan sebagai assist atau tiang pendulum.
Ground Gazebo diturunkan agar ayunan tidak terlihat menabrak lantai tanpa
menambah meja besar di bawah model.

Validasi headless terbaru disimpan di:

- `data_exports/lqr_smooth_noassist_v2_20260512.csv`
- `data_exports/real_smooth_noassist_v3_20260512.csv`
- `data_exports/pid_smooth_noassist_20260512.csv`
...
| Workspace | Sampel `BALANCE` | Tail strict 300 sampel akhir | Peak swing/catch | Peak balance | Assist engsel |
| --- | ---: | ---: | ---: | ---: | ---: |
| `lqr` | 3352 | 300 | `115.04 N` | `1.25 N` | `0.0 Nm` |
| `real` | 3223 | 300 | `130.00 N` | `12.12 N` | `0.0 Nm` |
| `pid` | 3446 | 300 | `94.72 N` | `0.93 N` | `0.0 Nm` |
...
mengganti teorinya, tetapi membuat profil no-assist lebih mirip alat asli:
force swing lebih kecil, ayunan lebih banyak, catch lebih dekat ke tegak, dan
balance tidak menghentak cart. Default terbaru memakai
`swing_force_limit_n = 75 N`, `catch_force_limit_n = 130 N`,
`balance_force_limit_n = 180 N`, dan `effort_limit_n = 200 N`. Gate capture
menunggu lebih lama dengan `swing_min_top_passes_before_catch = 5`,
`swing_min_energy_build_time_s = 10.0`, dan
`swing_energy_ready_ratio = 0.96`.

Validasi bersih menunjukkan ketiga workspace tetap masuk `BALANCE` dan stabil
di 300 sampel terakhir tanpa assist engsel. Peak saat balance turun jauh
dibanding profil lama; peak terbesar sekarang muncul di fase swing/catch untuk
menangkap pendulum, bukan untuk menahan pendulum tegak secara terus-menerus.
