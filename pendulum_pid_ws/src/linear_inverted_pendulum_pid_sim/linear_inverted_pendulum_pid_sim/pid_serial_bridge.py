...
class SimSerialBridge(Node):
    """PID Gazebo bridge for the STM32 serial protocol used by main.py."""

    def __init__(self):
        super().__init__("pendulum_pid_serial_bridge")

        self.declare_parameter("serial_symlink", "/tmp/pendulum_pid_serial")
        ...
        self.declare_parameter("balance_capture_cart_vel_mps", 0.8)
        ...
        self.declare_parameter("balance_immediate_capture_cart_vel_mps", 0.7)
        ...
        self.declare_parameter("balance_auto_lock_cart_vel_mps", 0.8)
        ...
        self.declare_parameter("balance_centering_force_gain", 120.0)
        self.declare_parameter("balance_cart_damping_force_gain", 35.0)
        ...
        self.declare_parameter("balance_theta_kd", 28.0)
        self.declare_parameter("balance_theta_ki", 4.0)
        ...
        self.balance_theta_ki = float(self.get_parameter("balance_theta_ki").value)
        ...
        self.get_logger().info(
            "PID balance controller active: theta P/I/D plus cart-centering PD."
        )
    ...
    def _swing_up_energy_command_locked(self, theta_top, dt, now):
        ...
        # Di sekitar posisi atas, gunakan PID yang sama untuk memperlambat
        # ayunan sebelum masuk balance penuh.
        if (
    ...
    def _upright_pid_force_locked(self, theta_top, dt, force_limit_n):
        ...
        if abs(theta_top) > self.balance_capture:
            self.theta_integral_rad_s *= clamp(1.0 - 4.0 * dt, 0.0, 1.0)
        center_scale = clamp(
        ...
        theta_integral_force_gain = abs(self.gains["K_X_INT"]) * (
            self.balance_theta_ki / 0.08
        )
