...
class SimSerialBridge(Node):
    """Bridge Gazebo joint states to the STM32 serial protocol used by main.py."""

    def __init__(self):
        super().__init__("pendulum_lqr_serial_bridge")

        self.declare_parameter("serial_symlink", "/tmp/pendulum_lqr_serial")
        ...
        self.declare_parameter("cart_effort_rate_limit_nps", 2200.0)
        ...
        self.declare_parameter("swing_gain", 1.80)
        self.declare_parameter("swing_centering_gain", 1.80)
        self.declare_parameter("swing_damping_gain", 0.70)
        self.declare_parameter("swing_kick_mps", 0.82)
        self.declare_parameter("swing_kick_force_n", 18.0)
        self.declare_parameter("swing_min_pump_force_n", 5.0)
        self.declare_parameter("swing_min_pump_force_extra_n", 10.0)
        self.declare_parameter("swing_force_rate_limit_nps", 220.0)
        self.declare_parameter("swing_force_limit_n", 75.0)
        self.declare_parameter("swing_min_top_passes_before_catch", 5)
        self.declare_parameter("swing_min_energy_build_time_s", 10.0)
        self.declare_parameter("swing_energy_ready_ratio", 0.96)
        self.declare_parameter("swing_top_pass_angle_deg", 80.0)
        self.declare_parameter("balance_capture_deg", 7.0)
        self.declare_parameter("balance_capture_rate_rad_s", 0.6)
        ...
        self.declare_parameter("balance_capture_cart_vel_mps", 0.6)
        self.declare_parameter("balance_immediate_capture_deg", 1.2)
        self.declare_parameter("balance_immediate_capture_rate_rad_s", 0.5)
        self.declare_parameter("balance_immediate_capture_cart_pos_m", 0.16)
        self.declare_parameter("balance_immediate_capture_cart_vel_mps", 0.6)
        self.declare_parameter("balance_auto_lock_enabled", True)
        self.declare_parameter("balance_auto_lock_angle_deg", 2.0)
        self.declare_parameter("balance_auto_lock_rate_rad_s", 0.6)
        self.declare_parameter("balance_auto_lock_cart_pos_m", 0.16)
        self.declare_parameter("balance_auto_lock_cart_vel_mps", 0.6)
        self.declare_parameter("balance_auto_lock_time_s", 0.12)
        self.declare_parameter("catch_region_deg", 70.0)
        self.declare_parameter("catch_region_rate_rad_s", 10.0)
        self.declare_parameter("catch_force_limit_n", 130.0)
        ...
        self.declare_parameter("balance_lqr_force_scale", 1.0)
        self.declare_parameter("balance_integral_force_gain", 0.2)
        self.declare_parameter("balance_centering_force_gain", 80.0)
        self.declare_parameter("balance_cart_damping_force_gain", 25.0)
        self.declare_parameter("balance_center_force_limit_n", 20.0)
        ...
        self.declare_parameter("balance_theta_kp", 95.0)
        self.declare_parameter("balance_theta_kd", 24.0)
        ...
        self.declare_parameter("lqr_q_x_dot", 10.0)
        self.declare_parameter("lqr_q_theta", 500.0)
        self.declare_parameter("lqr_q_theta_dot", 200.0)
        self.declare_parameter("lqr_r", 0.12)
        ...
        self.declare_parameter("effort_limit_n", 200.0)
        ...
        self.balance_lqr_force_scale = clamp(
            float(self.get_parameter("balance_lqr_force_scale").value),
            0.05,
            1.0,
        )
        ...
        self.balance_center_force_limit_n = float(
            self.get_parameter("balance_center_force_limit_n").value
        )
        ...
        if self.balance_lqr_gain is not None:
            self.get_logger().info(
                "Active LQR upright controller gain (force input): "
                + ", ".join(f"{gain:.2f}" for gain in self.balance_lqr_gain)
            )
        else:
            self.get_logger().info(
                "LQR disabled; using fallback PID-like upright controller."
            )
    ...
    def _balance_force_locked(self, theta_top, dt):
        if self.balance_use_lqr and self.balance_lqr_gain is not None:
            return self._state_feedback_force_locked(
                self.balance_lqr_gain,
                theta_top,
                dt,
                self.balance_force_limit_n,
                self.balance_lqr_force_scale,
            )
    ...
    def _state_feedback_force_locked(
        self, gain, theta_top, dt, force_limit_n, force_scale=1.0
    ):
        x_error_m = self.cart_x_m - self.x_center_m
        self.x_integral_cm_s = clamp(
            self.x_integral_cm_s + x_error_m * dt,
        ...
        state = (
            x_error_m,
            self.cart_v_mps,
            theta_top + theta_center_bias,
            self.pendulum_vel_radps,
        )
        lqr_force = -sum(
            gain_value * state_value for gain_value, state_value in zip(gain, state)
        )
        center_force = (
            self.balance_centering_force_gain * x_error_m
            + self.balance_cart_damping_force_gain * self.cart_v_mps
            + self.balance_integral_force_gain * self.x_integral_cm_s
        )
        center_force = clamp(
            center_force,
            -self.balance_center_force_limit_n,
            self.balance_center_force_limit_n,
        )
        force = lqr_force * force_scale + center_force
        force = self._soft_rail_guard_force_locked(force, force_limit_n)
        return clamp(force, -force_limit_n, force_limit_n)
